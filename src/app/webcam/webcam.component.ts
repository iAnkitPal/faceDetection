...
export class WebcamComponent implements OnInit {
  ...
  startVideo() {
    ...
    navigator.mediaDevices.getUserMedia({ video: {}, audio: false }).then((stream:any) => {
  ...
  async detect_Faces() {
    this.elRef.nativeElement.querySelector('video').addEventListener('play', async () => {
     this.canvas = await faceapi.createCanvasFromMedia(this.videoInput); this.canvasEl = this.canvasRef?.nativeElement;
